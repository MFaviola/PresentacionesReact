...
namespace SIMEXPRO.DataAccess.Repositories.Adua
{
    public class PersonaJuridicaRepository : IRepository<tbPersonaJuridica>
    {
        ...
        public RequestStatus InsertarCodigoj(string correo, string codigo)
        {
            using (var db = new SqlConnection(SIMEXPRO.ConnectionString))
            {
                var parametro = new DynamicParameters();
                parametro.Add("peju_CorreoElectronico", correo);
                parametro.Add("peju_ValidarCorreo", codigo);

                var result = db.Execute(ScriptsDataBase.ValidarCorreoj2,
                    parametro,
        ...
        public RequestStatus ConfirmarCodigoj(string codigo)
        {
            using (var db = new SqlConnection(SIMEXPRO.ConnectionString))
            {
                var parametro = new DynamicParameters();
                ...
                parametro.Add("peju_FechaModificacion", DateTime.Now);

                var result = db.Execute(ScriptsDataBase.ConfirmarCodigoj,
                    parametro,
                     commandType: CommandType.StoredProcedure
                    );
        ...
        public RequestStatus InsertarCodigoj2(string correo, string codigo)
        {
            using (var db = new SqlConnection(SIMEXPRO.ConnectionString))
            {
                var parametro = new DynamicParameters();
                parametro.Add("peju_CorreoElectronicoAlternativo", correo);
                parametro.Add("peju_ValidarCorreo2", codigo);
        ...
        public RequestStatus ConfirmarCodigoj2(string codigo)
        {
            using (var db = new SqlConnection(SIMEXPRO.ConnectionString))
            {
                var parametro = new DynamicParameters();
                ...
                var result = db.Execute(ScriptsDataBase.ConfirmarCodigoj2,
                    parametro,
                     commandType: CommandType.StoredProcedure
                    );
